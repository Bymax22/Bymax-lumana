...
type Vehicle = { id: string | number; make?: string; model?: string; year?: number; price?: number; status?: string; images?: unknown[]; imageUrl?: string };
...
function InventoryRow({ vehicle }: { vehicle: Vehicle }) { const status = vehicle.status || 'Available'; const imageUrl = getFirstMediaUrl(vehicle); return <Link href={`/seller/vehicles/${vehicle.id}`} className="flex min-w-0 items-center justify-between gap-4 py-4 transition hover:bg-white/[0.02]"><div className="flex min-w-0 items-center gap-3"><div className="flex h-14 w-20 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-[#1a1e20] text-slate-500">{imageUrl ? <img src={imageUrl} alt={`${vehicle.make || 'Vehicle'} ${vehicle.model || ''}`} className="h-full w-full object-cover" loading="lazy" /> : <CarFront size={20} />}</div><div className="min-w-0"><p className="truncate text-sm font-semibold text-slate-200">{vehicle.make || 'Vehicle'} {vehicle.model || ''}</p><p className="mt-1 text-xs text-slate-500">{vehicle.year || 'Year not set'} {vehicle.price ? `· $${Number(vehicle.price).toLocaleString()}` : ''}</p></div></div><span className="shrink-0 rounded-full bg-emerald-400/10 px-3 py-1 text-xs font-medium text-emerald-300">{status}</span></Link>; }
