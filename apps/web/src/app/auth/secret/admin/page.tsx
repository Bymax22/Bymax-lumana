...
export default function SecretAdminSignup() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      await authRegister({ name, email, password, role: 'ADMIN' });
  ...
  return (
    <section className="rounded-[24px] bg-[#0d0d0d] p-8 shadow-[0_30px_60px_rgba(0,0,0,0.35)]">
      <h1 className="text-2xl font-bold text-white">Secure Admin Signup</h1>
      ...
      <form onSubmit={handleSubmit} className="mt-6 space-y-4">
        ...
        <button disabled={loading} className="rounded-[12px] bg-red-600 px-4 py-2 text-white">{loading ? 'Creating...' : 'Create Admin'}</button>
